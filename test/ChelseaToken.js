...
contract('ChelseaToken', function(accounts){
  it('initializes the contract with the contract values', function(){
    return ChelseaToken.deployed().then(function(instance) {
      tokenInstance = instance;
      return tokenInstance.name();
    }).then(function(name) {
      assert.equal(name, 'Chelsea Token', 'has the correct name.');
      return tokenInstance.symbol();
    }).then(function(symbol){
      assert.equal(symbol, 'CHELSEA', 'has the correct symbol');
      return tokenInstance.standard();
    }).then(function(standard){
      assert.equal(standard, 'Chelsea Token v1.0', 'has the correct standard');
    })
  });
  ...
  it('allocates the inital supply upon development', function(){
    return ChelseaToken.deployed().then(function(instance) {
      ...
      assert.equal(totalSupply.toNumber(), 10000, 'set the total supply to 10,000,000');
      return tokenInstance.balanceOf(accounts[0]);
    }).then(function(adminBalance) {
      assert.equal(adminBalance.toNumber(), 10000, 'it allocates the inital supply to the admin account')
    })
  });
  ...
  it('transfers token ownership', function() {
    return ChelseaToken.deployed().then(function(instance) {
      ...
      return tokenInstance.transfer.call(accounts[1], 250, {
        from: accounts[0]
      })
      ...
      assert.equal(balance.toNumber(), 9750, 'deducts the amount from the sending account');
    })
  });
  ...
  it('approves tokens for delegate transfer', function() {
    return ChelseaToken.deployed().then(function(instance) {
      ...
      assert.equal(receipt.logs.length, 1, 'triggers approve event');
  ...
  it('handles delegated token transfers', function() {
    return ChelseaToken.deployed().then(function(instance){
      ...
      return tokenInstance.transferFrom(fromAccount, toAccount, 999999,{
        from: spendingAccount
      });
    }).then(assert.fail).catch(function(error){
      assert(error.message.indexOf('revert') >= 0, 'cannot transfer value larger than balance');
      // Try transferring something larger than the approved mount
      ...
      assert.equal(receipt.logs[0].args._to, toAccount, 'logs the account the tokens are transfer to');
